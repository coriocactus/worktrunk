...
**Decision: Two-tier completion system**
1. **Tier 1 (Static)**: Command and flag completion via `clap_complete`
2. **Tier 2 (Dynamic)**: Branch and worktree completion via callback protocol
...
User presses [TAB]
       ↓
Shell-specific completion script (generated by: wt completion <shell>)
       ↓
Static path: subcommands/flags (from clap metadata)
       ↓
Dynamic path: wt __complete <args...>
       ↓
Rust queries git and returns: branch names, worktree paths, etc.
```

### Implementation Phases

**Phase 1: Static Completion** (Quick Win - 2-4 hours)
- [ ] Add `clap_complete` dependency to Cargo.toml
- [ ] Create `Shell` enum compatible with `clap_complete::Shell`
- [ ] Add `Completion` subcommand: `wt completion <shell>`
- [ ] Implement `handle_completion()` using `clap_complete::generate()`
- [ ] Test completion generation for Fish, Bash, Zsh
- [ ] Update README with installation instructions

**Phase 2: Git Primitives for Dynamic Completion** (Foundation - 2-3 hours)
- [ ] Add `get_all_branches()` to `src/git.rs`
  - Uses: `git branch --format='%(refname:short)'`
  - Returns: `Vec<String>` of branch names
- [ ] Add `get_available_branches()` helper
  - Filters out branches that already have worktrees
  - Used for `wt switch` completion
- [ ] Add unit tests for branch listing functions
- [ ] Performance test: ensure <100ms for repos with 100+ branches

**Phase 3: Dynamic Completion Backend** (Core - 4-6 hours)
- [ ] Add hidden `__complete` command to `Commands` enum
  - Mark with `#[command(hide = true)]`
  - Takes trailing args: `args: Vec<String>`
- [ ] Implement `CompletionContext` enum:
  - `SwitchBranch` - complete available branches
  - `PushTarget` - complete all branches
  - `MergeTarget` - complete all branches
  - `BaseFlag` - complete all branches for `--base`
  - `Unknown` - no completions
- [ ] Implement `parse_completion_context()`:
  - Parse command line: `["wt", "switch", "feat"]`
  - Determine what position we're completing
  - Return appropriate `CompletionContext`
- [ ] Implement context-specific completion functions:
  - `complete_available_branches()` - for `switch`
  - `complete_all_branches()` - for `push`, `merge`, `--base`
- [ ] Implement `handle_complete()` main dispatcher
- [ ] Add unit tests for context parsing
- [ ] Add integration tests for completion output

**Phase 4: Shell Template Enhancement** (Polish - 3-4 hours)
- [ ] Enhance `templates/fish.fish`:
  - Add `__wt_complete` function that calls `wt __complete`
  - Add `complete -c wt` rules for dynamic completion
  - Test with real Fish shell
- [ ] Enhance `templates/bash.sh`:
  - Add `_wt_complete()` bash completion function
  - Register with `complete -F _wt_complete wt`
  - Test with real Bash shell
- [ ] Create `templates/zsh.zsh`:
  - Add `_wt_complete()` zsh completion function
  - Add zsh-specific completion registration
  - Test with real Zsh shell
- [ ] Update `shell.rs` to include completion code in templates
- [ ] Test all three shells with various completion scenarios

**Phase 5: Integration & Testing** (Validation - 2-3 hours)
- [ ] Manual testing checklist:
  - [ ] Fish: `wt sw<TAB>` shows `switch`
  - [ ] Fish: `wt switch <TAB>` shows available branches
  - [ ] Fish: `wt push <TAB>` shows all branches
  - [ ] Bash: same as Fish tests
  - [ ] Zsh: same as Fish tests
- [ ] Performance testing:
  - [ ] Measure `wt __complete` response time
  - [ ] Target: <100ms for typical repos
  - [ ] Test with repo containing 100+ branches
- [ ] Edge case testing:
  - [ ] Completion in non-git directory (should fail gracefully)
  - [ ] Completion with no branches
  - [ ] Completion with partial branch names
- [ ] Documentation:
  - [ ] Update README with completion installation
  - [ ] Add performance notes
  - [ ] Document `wt completion` usage

### Completion Context Logic

```rust
// Example completion contexts based on command line:

["wt", "switch", "feat"]
→ CompletionContext::SwitchBranch
→ Output: branches without worktrees matching "feat*"

["wt", "push", "ma"]
→ CompletionContext::PushTarget
→ Output: all branches matching "ma*"

["wt", "switch", "--create", "new", "--base", "dev"]
→ CompletionContext::BaseFlag
→ Output: all branches matching "dev*"
```

### Performance Requirements

- **Target:** <100ms for completion response
- **Acceptable:** <200ms
- **Optimization strategies (if needed):**
  - Cache branch list with 5-10 second TTL
  - Read `.git/refs/heads/*` directly instead of `git branch`
  - Stream results instead of collecting to vector
...
**Note:** Users must regenerate completion scripts after updating `wt`.
...
#[test]
fn test_complete_available_branches() {
    // Create test repo with branches and worktrees
    // Verify only available branches returned
}
```
...
# (requires manual testing in real shells)
```
...
|----------|--------|-----------|
| Static library | `clap_complete` | Official, well-integrated with clap |
| Dynamic protocol | Callback via `__complete` | Proven by gh, kubectl, git |
| Performance target | <100ms | Perceived as instant by users |
| Command name | `wt completion <shell>` | Industry standard convention |
| Hidden command | `wt __complete` | Prevents accidental user invocation |
